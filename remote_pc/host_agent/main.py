...
async def run_host_agent():
    auth = HostAuth()
    # Default demo Google user ID if not logged in
    ...
    while True:
        try:
            async with websockets.connect(SERVER_URI) as ws:
                ...
                # Task for receiving command messages from Mobile client
                    ...
                    try:
                        async for raw_msg in ws:
                            ...
                            if msg_type == "input_event":
                                ...
                                # Notify updated resolution to server/client
                ...
                # Loop for streaming video frames / low data heartbeat
                while True:
                    if capturer.is_background:
                        # Low-data heartbeat mode (~1 KB/s)
                        await ws.send(json.dumps({"type": "ping", "device_id": auth.device_id}))
                        await asyncio.sleep(2.0)
                    else:
                        # Foreground active streaming mode (High FPS)
